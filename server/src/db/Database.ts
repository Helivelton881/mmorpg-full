 ...
 * Production path: Render -> private Supabase Edge Function -> Supabase Postgres.
 * Optional direct PostgreSQL path remains available for local/self-hosted use.
 * Development fallback: in-memory when no persistent backend is configured.
 */
import { Pool } from 'pg';
...
export class Database {
  ...
  async connect() {
    this.gatewayUrl = process.env.DB_GATEWAY_URL || '';
    this.gatewaySecret = process.env.DB_GATEWAY_SECRET || '';

    if (this.gatewayUrl && this.gatewaySecret) {
      this.gatewayEnabled = true;
      try {
        const health = await this.gatewayCall<{ok:boolean}>('health', {});
        if (!health.ok) throw new Error('Gateway health check failed');
        this.useMemory = false;
        console.log('[DB] Connected to Supabase persistence gateway');
        return;
      } catch (err:any) {
        console.error('[DB] Supabase gateway unavailable:', err?.message || err);
        throw err;
      }
    }
    ...
    if (connectionString) {
      ...
      this.useMemory = false;
      console.log('[DB] Connected directly to PostgreSQL');
      return;
    }

    if (process.env.NODE_ENV === 'production') {
      throw new Error('No persistent database configured in production');
    }

    console.warn('[DB] No persistent database configured -> in-memory development mode');
  ...
  private async gatewayCall<T>(op:string, payload:Record<string,unknown>):Promise<T> {
    const response = await fetch(this.gatewayUrl, {
      method:'POST',
      headers:{
        'content-type':'application/json',
        'x-game-server-key':this.gatewaySecret,
      },
    ...
    if (!response.ok) {
      throw new Error(parsed?.error || `Gateway HTTP ${response.status}`);
    }
    return parsed as T;
  }
  ...
  private async ensureSchema() {
    ...
    await this.pool.query(`
      ...
      CREATE TABLE IF NOT EXISTS characters (
        ...
        class_name VARCHAR(32) NOT NULL DEFAULT 'guerreiro',
        level INTEGER NOT NULL DEFAULT 1,
        experience INTEGER NOT NULL DEFAULT 0,
        pos_x DOUBLE PRECISION NOT NULL DEFAULT 500,
  ...
  async register(username:string,password:string):Promise<{ok:boolean;error?:string;userId?:string}> {
    if (this.gatewayEnabled) {
      return this.gatewayCall('register',{username,password});
    }
    ...
    if(this.useMemory&&this.memory){
      ...
      this.memory.users.set(id,user);this.memory.usernameIndex.set(clean,id);
      return{ok:true,userId:id};
    }

    try{
      const res=await this.pool!.query('INSERT INTO users (username,password_hash) VALUES ($1,$2) RETURNING id',[clean,hash]);
  ...
  async login(username:string,password:string):Promise<{ok:boolean;error?:string;userId?:string;token?:string}> {
    if(this.gatewayEnabled){
      return this.gatewayCall('login',{username,password});
    }

    const clean=username.trim().toLowerCase();
    let user:UserRow|null=null;
    if(this.useMemory&&this.memory){
      const id=this.memory.usernameIndex.get(clean);if(id)user=this.memory.users.get(id)||null;
    }else{
      const res=await this.pool!.query('SELECT * FROM users WHERE username=$1',[clean]);user=res.rows[0]||null;
    }
    if(!user||!(await bcrypt.compare(password,user.password_hash)))return{ok:false,error:'Usuário ou senha inválidos'};
    const token=randomBytes(32).toString('hex'),expiresAt=Date.now()+7*24*60*60*1000;
    if(this.useMemory&&this.memory){
      this.memory.sessions.set(token,{userId:user.id,expiresAt});user.last_login=new Date();
    }else{
      await this.pool!.query(
  ...
  async validateToken(token:string):Promise<string|null> {
    if(this.gatewayEnabled){
      const res=await this.gatewayCall<{userId:string|null}>('validateToken',{token});
      return res.userId||null;
    }
    if(!token)return null;
    if(this.useMemory&&this.memory){
      const s=this.memory.sessions.get(token);
      if(!s||s.expiresAt<Date.now()){this.memory.sessions.delete(token);return null;}
      return s.userId;
    }
    const res=await this.pool!.query('SELECT user_id FROM sessions WHERE token=$1 AND expires_at>NOW()',[token]);
    return res.rows[0]?.user_id||null;
  }

  async getCharacters(userId:string):Promise<CharacterRow[]> {
    if(this.gatewayEnabled){
      const res=await this.gatewayCall<{characters:CharacterRow[]}>('getCharacters',{userId});
  ...
  async createCharacter(userId:string,name:string,className:string):
    ...
    if(this.gatewayEnabled){
      return this.gatewayCall('createCharacter',{userId,name,className});
    }

    const cleanName=name.trim().slice(0,16);
    if(cleanName.length<2)return{ok:false,error:'Nome muito curto'};
    const valid=['guerreiro','arqueiro','mago','clerigo','paladino','ladino','barbaro','monge','necromante'];
    const cls=valid.includes(className)?className:'guerreiro';
    const defaultInv=JSON.stringify([{itemId:'potion_hp',quantity:3},{itemId:'potion_mp',quantity:2}]);
    ...
    if(this.useMemory&&this.memory){
      ...
      this.memory.characters.set(id,char);this.memory.charNameIndex.set(cleanName.toLowerCase(),id);
      return{ok:true,character:char};
    }

    try{
      const count=await this.pool!.query('SELECT COUNT(*)::int AS cnt FROM characters WHERE user_id=$1',[userId]);
      if(count.rows[0].cnt>=3)return{ok:false,error:'Máximo de 3 personagens'};
      const res=await this.pool!.query(
        `INSERT INTO characters
        (user_id,name,class_name,pos_x,pos_y,gold,inventory_json,equipment_json,quests_json)
  ...
  async getCharacter(charId:string,userId:string):Promise<CharacterRow|null> {
    if(this.gatewayEnabled){
      const res=await this.gatewayCall<{character:CharacterRow|null}>('getCharacter',{charId,userId});
      return res.character||null;
    }
    if(this.useMemory&&this.memory){
      const c=this.memory.characters.get(charId);return c&&c.user_id===userId?c:null;
    }
  ...
  async deleteCharacter(charId:string,userId:string):Promise<{ok:boolean;error?:string}> {
    if(this.gatewayEnabled){
      return this.gatewayCall('deleteCharacter',{charId,userId});
    }
    if(this.useMemory&&this.memory){
      const c=this.memory.characters.get(charId);
      if(!c||c.user_id!==userId)return{ok:false,error:'Personagem não encontrado'};
      this.memory.charNameIndex.delete(c.name.toLowerCase());this.memory.characters.delete(charId);return{ok:true};
    }
    const res=await this.pool!.query('DELETE FROM characters WHERE id=$1 AND user_id=$2 RETURNING id',[charId,userId]);
    return res.rowCount?{ok:true}:{ok:false,error:'Personagem não encontrado'};
  }
  ...
  async saveProgress(charId:string,data:{
    ...
    if(this.gatewayEnabled){
      await this.gatewayCall('saveProgress',{charId,data});
      return;
    }
    ...
    sets.push('updated_at=NOW()');vals.push(charId);
